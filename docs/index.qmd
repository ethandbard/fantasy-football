...
Output lands in `docs/_site`. That folder is static HTML.

**Public site.** Push to `main`. The workflow in `.github/workflows/quarto-publish.yml` renders the site and deploys it to the `gh-pages` branch. That updates https://ethandbard.github.io/fantasy-football/ and https://fantasy-docs.ethandbard.com/.

In the GitHub repo, set **Settings → Pages** to deploy from `gh-pages`. A Cloudflare Worker on the custom hostname fetches that GitHub Pages copy.
